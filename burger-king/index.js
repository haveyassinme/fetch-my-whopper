...
// import { de } from "faker/lib/locales/de.js"
	...
	await page.evaluate(selectorNumber => {
		...
		arrayRows.forEach(row => {
			if (row.id) {
				...
				document
					.querySelector(`#${row.id} > .Opt${selectorNumber} input`)
					.click()
			}
		})
	}, selectorNumber)
...
export const fetchWhopper = async function (storeNumber = "21019", date, time) {
	...
	try {
		...
		await page.waitForSelector(
			"#content > #surveyEntryForm > #surveyQuestions > #Buttonholder > #NextButton"
		)
		await page.click(
			"#content > #surveyEntryForm > #surveyQuestions > #Buttonholder > #NextButton"
		)
		...
		await page.waitForSelector(
			"#content > #surveyEntryForm > #surveyQuestions > .IndexText01 > #SurveyCode"
		)
		await page.type(
			"#content > #surveyEntryForm > #surveyQuestions > .IndexText01 > #SurveyCode",
			storeNumber
		...
		await page.select(
			"#content > #surveyEntryForm > #surveyQuestions > .IndexText01 > #InputDay",
			dateFormatted.get("date").toString()
		)
		await page.select(
			"#content > #surveyEntryForm > #surveyQuestions > .IndexText01 > #InputMonth",
			formatMonth(dateFormatted.get("month") + 1)
		)
		...
		await page.evaluate(time => {
			...
			document.querySelector(
				"#content > #surveyEntryForm > #surveyQuestions > .IndexText01 > #InputHour"
			).value = hours
			document.querySelector(
				"#content > #surveyEntryForm > #surveyQuestions > .IndexText01 > #InputMinute"
			).value = minutes
		}, time)
		...
		await page.waitForSelector(
			"#content > #surveyForm > #surveyQuestions > #Buttonholder > #NextButton"
		)
		await page.click(
			"#content > #surveyForm > #surveyQuestions > #Buttonholder > #NextButton"
		)
		...
		const x = await page.evaluate(
			() => document.querySelector(".ValCode").innerText
		)
